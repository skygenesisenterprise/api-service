...
use russh::{server::{self, Session}, ChannelId, CryptoVec};
...
impl SshAuthHandler {
    ...
    pub async fn authenticate_public_key(
        &self,
        user: &str,
        public_key: &russh_keys::PublicKey,
    ) -> Result<(), russh::Error> {
...
pub struct SshSessionHandler {
    ...
    shell_handler: Arc<SshAuthHandler>,
    session_manager: Arc<SshAuthHandler>,
    id: usize,
...
pub struct SshServer {
    ...
    shell_handler: Arc<SshAuthHandler>,
    session_manager: Arc<SshAuthHandler>,
    host_keys: Vec<ssh_key::PrivateKey>,
    id: AtomicUsize,
}
...
impl SshServer {
    ...
    async fn load_host_keys(
        vault_client: Arc<VaultClient>,
    ) -> Result<Vec<ssh_key::PrivateKey>, Box<dyn std::error::Error + Send + Sync>> {
                ...
                for key_str in keys {
                    match russh_keys::decode_secret_key(&key_str, None) {
                        ...
                        Err(e) => {
                            ...
                            let new_key = ssh_key::PrivateKey::random(&mut rand::thread_rng(), ssh_key::Algorithm::Ed25519).unwrap();
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let server = self.run(config, addr).await?;
        server.await;
